...
export default function Navbar({
  ...
  return (
    <header className="relative border-b border-slate-200/70 bg-white/80 backdrop-blur dark:border-slate-800 dark:bg-slate-950/80">
        ...
        <nav className="hidden items-center gap-6 text-sm font-medium text-slate-600 dark:text-slate-400 md:flex">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="transition hover:text-primary-700 dark:hover:text-primary-400"
            >
              {link.label}
            </a>
          ))}
          ...
          {isLoggedIn ? (
            <div className="flex items-center gap-4">
              <span className="max-w-[8rem] truncate text-slate-500 dark:text-slate-400">
                {displayName}
              </span>
              <button
                type="button"
          ...
          <div className="flex flex-col gap-1">
            {links.map((link) => (
              <a
                ...
                className="rounded-lg px-3 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-800"
              >
                {link.label}
              </a>
            ))}
